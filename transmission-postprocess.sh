...
--def artwork=false ut_kind=multi --def ignore=Sonarr --def ut_label=Movie "ut_dir=$TORRENT_PATH" "ut_title=$TORRENT_NAME" subtitles=$SUBLANG \
--def "movieFormat={fn =~ /2160p|4K|4k|UHD/ ? '$RCLONE_DIR/Films 4K' : fn =~ /3D|3d|HSBS|3dhsbs|H-SBS|3dhou|H-OU/ ? '$RCLONE_DIR/Films 3D' : vf =~ /1080p/ ? '$RCLONE_DIR/Films HD/1080p' : vf =~ /720p/ ? '$RCLONE_DIR/Films HD/720p' : vf =~ /576p/ ? '$RCLONE_DIR/Films SD/576p' : vf =~ /480p/ ? '$RCLONE_DIR/Films SD/480p' : vf =~ /360p/ ? '$RCLONE_DIR/Films SD/360p' : vf =~ /240p/ ? '$RCLONE_DIR/Films SD/240p' : '$RCLONE_DIR/Films SD/root'}/{n} ({y})/{n} ({y}) - {allOf{match}{s3d}{hd}{vc}{vf} join ' '}{' - pt'+pi}{'.'+lang}" \
